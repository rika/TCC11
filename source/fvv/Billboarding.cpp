...
void Billboarding::loadObjectData(int frame) {
  ...
  if (resourceData) {
    ...
    for (int i = 0; i < h.size(); i++) {
      tmpObjects.push_back(Object(h[i], cx[i], cy[i]));
    }
...
void Billboarding::renderVideoThumbs() {
  ...
  glLoadIdentity();
  double scrWidth  = 720.0;
  double scrHeight = 576.0;
  glOrtho(0.0, scrWidth, 0.0, scrHeight, -1.0, 1.0);

  glColor3f(1.0, 1.0, 1.0);
  for (int i = 0; i < numCameras; i++) {
    stringstream key;
    key << "camera" << i << "foreground";
    texManager->bindTexture(key.str());

    glBegin(GL_QUADS);
    glTexCoord2f(0.0, 1.0);
    glVertex3f(i * (scrWidth / numCameras), 0.0, 0.0);
    
    glTexCoord2f(1.0, 1.0);
    glVertex3f((i + 1) * (scrWidth / numCameras), 0.0, 0.0);
    
    glTexCoord2f(1.0, 0.0);
    glVertex3f((i + 1) * (scrWidth / numCameras), scrHeight / numCameras, 0.0);
    
    glTexCoord2f(0.0, 0.0);
    glVertex3f(i * (scrWidth / numCameras), scrHeight / numCameras, 0.0);
    glEnd();
  }
...
void Billboarding::renderFrame(int frame) {
  ...
  for (int i = 0; i < zOrdered.size(); i++) {
    if (dummyRender) {
      ...
      renderObject(objects[zOrdered[i].second]);
    }
  }
}

void Billboarding::renderObjectDummy(Object object) {
...
void Billboarding::renderObject(Object object) {
  ...
  if (foregroundMask) {
    if (camsToRender.size() == 1) {
      ...
      renderBillboard(a, b, c, d, object, camsToRender[0].first);        
    } else {
      for (int i = 0; i < camsToRender.size(); i++) {
	glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_ADD);
	glColor4f(0.0, 0.0, 0.0, camsToRender[i].second);
	renderBillboard(a, b, c, d, object, camsToRender[i].first);    
      }
    }
  } else {
    if (camsToRender.size() == 1) {

...
SceneModel::SceneModel(string scenarioDataPath) {
  ...
  cout << "Loading object tracking data" << endl;
  string path = scenarioData->getStringValue("objectDataPath");
  objectData = new ObjectData(path, startFrame, endFrame);
...
void SceneModel::on_realize() {
  ...
  billboarding = new Billboarding(texManager, scenarioData, vdtm, objectData);
  billboarding->setup();
